# Contributing to SAP Fiori Browser Automation
...
- **Broken selectors**: SAP Fiori updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real SAP Fiori instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SAP_FIORI_URL}/path/to/section`);
...
When you find working selectors for SAP Fiori, document them:

```javascript
// SAP Fiori selectors (verified YYYY-MM-DD)
...
SAP Fiori updates its UI periodically. When selectors break:
